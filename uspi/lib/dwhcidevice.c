...
#include "../../uspi/include/uspi/bcm2835.h"
...
boolean DWHCIDeviceInitialize (TDWHCIDevice *pThis)
{
	...
	_kernel_interrupt_register_irq_handler( INTERRUPT_IRQ_USB, DWHCIDeviceInterruptHandler, pThis );
	_kernel_interrupt_enable_irq( INTERRUPT_IRQ_USB );
...
// ACU - so far the rabbit hole arrives here (DWHCIDeviceControlMessage 3 tells us this call is falling over somehow, or blocking)
