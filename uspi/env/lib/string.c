...
void String2 (TString *pThis, const char *pString)
{
	...
	pThis->m_nSize = _kernel_strlen( pString ) + 1;

	pThis->m_pBuffer = (char *) malloc (pThis->m_nSize);

	_kernel_strcpy( pThis->m_pBuffer, pString );
}
...
size_t StringGetLength (TString *pThis)
{
	...
	return _kernel_strlen( pThis->m_pBuffer );
}
...
void StringAppend (TString *pThis, const char *pString)
{
	...
	if (pThis->m_pBuffer != 0)
	{
		pThis->m_nSize += _kernel_strlen( pThis->m_pBuffer );
	}
	pThis->m_nSize += _kernel_strlen( pString );

	char *pBuffer = (char *) malloc (pThis->m_nSize);

	if (pThis->m_pBuffer != 0)
	{
		_kernel_strcpy( pBuffer, pThis->m_pBuffer );
		free (pThis->m_pBuffer);
	}
	else
	{
	...
	_kernel_strcat( pBuffer, pString );
...
int StringCompare (TString *pThis, const char *pString)
{
	...
	return _kernel_strcmp( pThis->m_pBuffer, pString );
}
	...
	while (*pFormat != '\0')
	{
		if (*pFormat == '%')
		{
			...
			case 'd':
				...
				nLen = _kernel_strlen( NumBuf ) + (bMinus ? 1 : 0);
				if (bLeft)
				{
					if (bMinus)
					{
...
void StringPutString (TString *pThis, const char *pString)
{
	...
	size_t nLen = _kernel_strlen( pString );
	
	StringReserveSpace (pThis, nLen);
	
	_kernel_strcpy ( pThis->m_pInPtr, pString );
...
void StringReserveSpace (TString *pThis, size_t nSpace)
{
	...
	*pThis->m_pInPtr = '\0';
	_kernel_strcpy (pNewBuffer, pThis->m_pBuffer);
